...
public class QuestCalibration {
        ...
        private Translation2d calculateOffsetToRobot(Pose2d questRobotPose) {
                ...
                double x = ((angle.getCos() - 1) * displacement.getX() + angle.getSin() * displacement.getY())
                                / (2 * (1 - angle.getCos()));
                double y = ((-angle.getSin()) * displacement.getX() + (angle.getCos() - 1) * displacement.getY())
                                / (2 * (1 - angle.getCos()));
         ...
         */
        public Command determineOffsetToRobotCenter(DriveSubsystem drive, Pose2d questPoseSupplier) {
                return Commands.repeatingSequence(
                                ...
                                Commands.runOnce(
                                                () -> {
                                                        ...
                                                        Logger.recordOutput(
                                                                        "QuestCalibration/CalculatedOffset",
                                                                        calculatedOffsetToRobot);
                                                })
                                                .onlyIf(() -> questPoseSupplier.getRotation().getDegrees() > 30))
                                .finallyDo(
                                                () -> {
                                                        // Update current offset
                                                        Translation2d offset = calculateOffsetToRobot(
                                                        ...
                                                        calculatedOffsetToRobot = calculatedOffsetToRobot
                                                                        ...
                                                                        .plus(offset.div(calculateOffsetCount + 1));
                                                        calculateOffsetCount++;
                                                        Logger.recordOutput("QuestCalibration/CalculatedOffset",
                                                                        calculatedOffsetToRobot);
                                                });
        }
}